...
void init_adc() {
	...
	ADC1->CR &= ~(ADC_CR_ADEN); //turn off adc
	...
	wait_time = 20*(16000000/1000000); //wake up time :)
	while(wait_time != 0) {
		wait_time--;
	}
	...
	ADC1->CFGR &= 0xFFFFFFC7;
	ADC1->SQR1 &= ~(ADC_SQR1_L);
	ADC1->SQR1 |= 0x00000440;
	ADC1->SMPR1 = 0x4;
	ADC1->CFGR &= ~(ADC_CFGR_CONT);
	ADC1->CFGR &= ~(ADC_CFGR_EXTEN);
	ADC1->CR |= ADC_CR_ADEN; //turn adc back on
